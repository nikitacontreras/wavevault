...
import { Download, X, CheckCircle, AlertCircle, Loader2, Waves, ChevronDown, ChevronUp } from "lucide-react";
...
export const ActivityPanel: React.FC = () => {
    ...
    return (
        <div className={`fixed bottom-24 right-8 w-80 border rounded-2xl shadow-2xl overflow-hidden z-[90] animate-in slide-in-from-bottom-2 transition-all duration-300 ${isDark ? "bg-wv-sidebar border-white/10 text-white" : "bg-white border-black/10 text-black"} ${isMinimized ? "h-[54px]" : "max-h-[400px]"}`}>
            <div
                className={`flex items-center justify-between p-4 cursor-pointer select-none group/header ${!isMinimized && "border-b"} ${isDark ? "border-white/[0.08]" : "border-black/[0.08]"}`}
                onClick={() => setIsMinimized(!isMinimized)}
            >
                <div className="flex items-center gap-2">
                    <ActivityIcon size={16} className={`${isMinimized ? "text-blue-500" : "text-wv-gray"} transition-colors`} />
                    <h3 className={`text-sm font-bold ${isDark ? "text-white" : "text-black"}`}>
                        Actividad ({total})
                    </h3>
                </div>
                <div className={`p-1 rounded-md transition-colors ${isDark ? "group-hover/header:bg-white/5" : "group-hover/header:bg-black/5"}`}>
                    {isMinimized ? <ChevronUp size={14} className="text-wv-gray" /> : <ChevronDown size={14} className="text-wv-gray" />}
                </div>
            </div>

            {!isMinimized && (
                <div className="max-h-80 overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-1 duration-200">
                    {/* STEMS SECTION */}
